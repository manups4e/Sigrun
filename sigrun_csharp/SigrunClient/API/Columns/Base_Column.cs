...
namespace SigrunClient.API.Columns
{
	...
	public delegate void ItemSelect(MainMenu menu, BaseItem item, int itemIndex);

	public class Base_Column
	{
		...
		public bool ScrollNewStyle = true;

		internal bool visible => Parent != null && Parent.Visible && Parent.Parent != null && Parent.Parent.Visible;
		public List<BaseItem> Items { get; internal set; } = new List<BaseItem>();

		internal string _oldTitle;
		internal bool _oldShowArrow;
		internal bool _oldHideTabs;
		...
		public virtual int Index
		{
			...
			set
			{
				...
				if (visible && Parent.CurrentColumnIndex == (int)position)
					ClientMain.sigrun.CallFunction("SET_COLUMN_HIGHLIGHT", (int)position, index, false, false);
			}
		}
		...
		public bool ColumnVisible
		{
			...
			{
				columnVisible = value;
				ClientMain.sigrun.CallFunction("SHOW_COLUMN", (int)position, columnVisible);
			}
		}
		...
		public Base_Column(PM_COLUMNS position)
		{
			this.position = position;
		}

		public Base_Column(int position) : this((PM_COLUMNS)position) { }
		...
		public virtual void RemoveSlot(int idx)
		{
			...
			Items.RemoveAt(idx);
			if (visible)
				ClientMain.sigrun.CallFunction("REMOVE_SLOT", (int)position, idx, false, false);
			if (Items.Count > 0)
			{
				if (idx == this.index)
				...
				Items[index].Selected = true;
				if (visible && Parent.CurrentColumnIndex == (int)position)
					ClientMain.sigrun.CallFunction("SET_COLUMN_HIGHLIGHT", (int)position, idx, false, false);
			}
		}
		...
		{
			if (visible)
				ClientMain.sigrun.CallFunction("SET_COLUMN_FOCUS", (int)position, highlighted, moveFocus, prevHighlight);
		}
		...
		{
			ClientMain.sigrun.CallFunction("DISPLAY_DATA_SLOT", (int)position, 0, 0);
		}
		public void InitColumnScroll(bool visible, int visibleItems, int maxItems, int index)
		{
			if (visible)
			{
				ClientMain.sigrun.CallFunction("INIT_SCROLL_BAR", (int)position, visible, visibleItems, maxItems, index);
			}
		}

		public void SetColumnScroll(bool visible, int index)
		{
			if (visible)
			{
				ClientMain.sigrun.CallFunction("SET_SCROLL_BAR", (int)position, visible, index);
			}
		}
		public virtual void SetColumnScroll(int currentPosition, int maxPosition, int maxVisible = -1)
		{
			if (visible)
				ClientMain.sigrun.CallFunction("SET_COLUMN_SCROLL", (int)position, currentPosition, maxPosition, maxVisible);
		}
		...
		public virtual void SetColumnScroll(string caption, params object[] args)
		{
			if (visible)
			{
				BeginScaleformMovieMethod(ClientMain.sigrun.Handle, "SET_COLUMN_SCROLL");
