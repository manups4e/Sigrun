...
function Scaleform:CallHUDFunction(componentID, theFunction, ...)
    BeginScaleformMovieMethod(self.handle, "CONTAINER_METHOD")
    ScaleformMovieMethodAddParamInt(componentID)
    ScaleformMovieMethodAddParamTextureNameString(theFunction)
    local arg = { ... }
    if arg ~= nil then
        for i = 1, #arg do
...
function Scaleform:Render3D(pos, rot, scale)
    DrawScaleformMovie_3dSolid(self.handle, pos.x, pos.y, pos.z, rot.x, rot.y, rot.z, 2.0, 2.0, 1.0, scale.x, scale.y, scale.z, 2)
end

---Render the scaleform in 3D space with additive blending
function Scaleform:Render3DAdditive(pos, rot, scale)
    DrawScaleformMovie_3d(self.handle, pos.x, pos.y, pos.z, rot.x, rot.y, rot.z, 2.0, 2.0, 1.0, scale.x, scale.y, scale.z, 2)
end
